...
A Worker User Library template that exposes a Workers for LabVIEW application to gRPC clients written in LabVIEW, Python, C++, or any other supported language.
...
| `gRPC Server (template.proto).lvlib` | gRPC Server Worker. Creates the gRPC server and handles the bidirectional stream. |
| `gRPC Server Tester.lvclass` | Tester Worker. Calls the gRPC Server Worker and provides the front panel used to send and receive messages. |
| `Launcher - gRPC Server Tester.vi` | Launches the tester. |
...
The tester is designed to be used together with the **gRPC Client Worker Template Tester**, or with a client application written in another programming language that implements the same `template.proto` contract.

1. Run **Launcher - gRPC Server Tester.vi** to launch the tester, then press **Create gRPC Server** to create the server and begin listening for connections. The **Server Listening** LED lights when the server is ready.
2. Run **Launcher - gRPC Client Tester.vi** in the gRPC Client Worker Template project and press **Create gRPC Client**. The **Client Connected** LED lights when the client opens the stream.
3. Enter text in any **Msg_ToClient** control to send a message to the client. Messages received from the client appear under their corresponding **Msg_ToServer** indicators.
4. Press **Destroy gRPC Server** when finished to close the connection and stop the server.
